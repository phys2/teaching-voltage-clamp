...
0<c&&(c++,channelName=channelName+" ("+c.toString()+")");a.channelName=channelName;a.display=!0;a.values=[];b=$jscomp.makeIterator(b);for(c=b.next();!c.done;c=b.next())c=c.value,d={},d.referencePotential=c.referencePotential,e=c.currents.indexOf(Math.max.apply(null,c.currents.map(Math.abs))),5>e&&(e=5),d.averageCurrent=average(c.currents.slice(e-5,e+5)),a.values.push(d);ivMaxPointTotalMatrix.push(a);paintIvCanvas();$("#fitCurve").removeAttr("disabled");$("#eraseIvButton").removeAttr("disabled");$("#selectedChannelToFit").append($("<option>",
{value:ivMaxPointTotalMatrix.length-1,text:channelName}));"NaN"==$("#selectedChannelToFit").val()&&($("#selectedChannelToFit").children('option[value="NaN"]').remove(),$("#selectedChannelToFit").removeAttr("disabled"))}
...
d,2*ivMaxAmp,ivY0RelPos)),g=(f*d).toString(),0==f%6&&(g+=" nA"),b.fillText(g,0,getIvYValue(c,f*d,2*ivMaxAmp,ivY0RelPos)),b.moveTo(0,getIvYValue(c,-1*f*d,2*ivMaxAmp,ivY0RelPos)),b.lineTo(a,getIvYValue(c,-1*f*d,2*ivMaxAmp,ivY0RelPos)),g=(-1*f*d).toString(),b.fillText(g,0,getIvYValue(c,-1*f*d,2*ivMaxAmp,ivY0RelPos));b.stroke();drawConductanceGrid();document.getElementById("channelLegend").innerHTML="";d="--bs-green --bs-blue --bs-red --bs-yellow --bs-pruple --bs-orange --bs-cyan --bs-pink --bs-teal".split(" ");
for(f=0;f<ivMaxPointTotalMatrix.length;f++){ivMaxPointChannelMatrix=ivMaxPointTotalMatrix[f];g=document.createElement("div");g.classList.add("btn");g.classList.add("btn-sm");g.classList.add("mb-2");g.classList.add("me-2");g.style.setProperty("--channelColor","var("+d[f]+")");var h=ivMaxPointChannelMatrix.display;g.innerHTML='<button class="btn btn-sm channelTag'+(h?" displayed":"")+'" onclick="toogleChannelDisplay('+f+')" >'+ivMaxPointChannelMatrix.channelName+"</button>";document.getElementById("channelLegend").appendChild(g);
if(h){g=getComputedStyle(g).getPropertyValue("--channelColor");h=$jscomp.makeIterator(ivMaxPointChannelMatrix.values);for(var l=h.next();!l.done;l=h.next())l=l.value,b.beginPath(),b.fillStyle=g,b.arc(getIvXValue(a,l.referencePotential,200,ivX0RelPos),getIvYValue(c,l.averageCurrent,2*ivMaxAmp,ivY0RelPos),4,0,2*Math.PI,!1),b.fill();if("undefined"!==typeof ivMaxPointChannelMatrix.fitStatus){b.beginPath();b.setLineDash([]);b.strokeStyle=g;b.lineWidth="1.5";b.moveTo(getIvXValue(a,ivMaxPointChannelMatrix.fittedXValues[0],
200,ivX0RelPos),getIvYValue(c,ivMaxPointChannelMatrix.fittedYValues[0],2*ivMaxAmp,ivY0RelPos));for(h=1;h<ivMaxPointChannelMatrix.fittedXValues.length;h++)b.lineTo(getIvXValue(a,ivMaxPointChannelMatrix.fittedXValues[h],200,ivX0RelPos),getIvYValue(c,ivMaxPointChannelMatrix.fittedYValues[h],2*ivMaxAmp,ivY0RelPos));b.stroke();e.beginPath();e.strokeStyle=g;e.setLineDash([]);e.lineWidth="2.5";e.moveTo(getIvXValue(condWidth,ivMaxPointChannelMatrix.fittedXValues[0],200),getIvYValue(condHeight,ivMaxPointChannelMatrix.fittedCondYValues[0],
1.5,.833));for(g=1;g<ivMaxPointChannelMatrix.fittedXValues.length;g++)e.lineTo(getIvXValue(condWidth,ivMaxPointChannelMatrix.fittedXValues[g],200),getIvYValue(condHeight,ivMaxPointChannelMatrix.fittedCondYValues[g],1.5,.833));e.stroke()}}}}function numberOfNameOccurences(a){for(var b=0,c=$jscomp.makeIterator(ivMaxPointTotalMatrix),d=c.next();!d.done;d=c.next())d.value.channelName==a&&b++;return b}
...
function recordPassive(){var a=timerCounterToTime(passiveCounter),b=getPermeabilities(lastPotential,null),c=calculateMembranePotential(b),d=getJunctionPotential(),e=parseFloat($("#offsetPotentialInput").val()),f=getNoise(.5);lastPotential=c=d+e+f+c;b=calculateCurrentForGivenPotential(c,b);updateOutputFields(c,b);voltageClampCounter==6E4/deltaT?(clearInterval(passiveTimer),patchStatus=6,$("#stopRecording").attr("disabled","disabled")):(passiveCounter++,canvasPointsMatrix.push([a,c,b]),paintMainCanvas())}
...
function recordIvCurve(){var a=timerCounterToTime(ivCurveCounter),b=$jscomp.makeIterator(getClampPotential(a)),c=b.next().value;b=b.next().value;var d=getJunctionPotential(),e=parseFloat($("#offsetPotentialInput").val());d=d+e+c;b=getPermeabilities(c,b);b=calculateCurrentForGivenPotential(d,b);updateOutputFields(c,b);ivDataMatrix[ivCurveCounter]=[c,b];ivCurveCounter==6E4/deltaT?(clearInterval(ivCurveTimer),patchStatus=8,$("#stopRecording").attr("disabled","disabled"),$("#importIvData").removeAttr("disabled")):
(ivCurveCounter++,canvasPointsMatrix.push([a,c,b]),paintMainCanvas())}function getClampPotential(a){for(var b=parseInt($("#V_mem").val()),c=parseInt($("#v_0Input").val()),d=parseInt($("#deltaVInput").val()),e=0;11>e&&!(a<5.35*e+3.21);e++)if(a<5.35*(e+1))return[c+e*d,a-(5.35*e+3.21)];return[b,null]}
function updateWholeCellReadout(){var a=timerCounterToTime(bathTimerCounter),b=getPermeabilities(lastPotential,null),c=calculateMembranePotential(b),d=getJunctionPotential(),e=parseFloat($("#offsetPotentialInput").val()),f=getNoise(.5);lastPotential=c=d+e+f+c;b=calculateCurrentForGivenPotential(c,b);updateOutputFields(c,b);bathTimerCounter==6E4/deltaT?(bathTimerCounter=0,canvasPointsMatrix.length=0):(bathTimerCounter++,canvasPointsMatrix.push([a,c,b]),paintMainCanvas())}
...
function drawRecord(a,b,c,d,e){var f=document.getElementById(a);a=f.getContext("2d");a.setTransform(1,0,0,1,0,0);a.scale(dpi,dpi);var g=f.getBoundingClientRect().height;f=f.getBoundingClientRect().width;a.lineWidth="2";a.setLineDash([]);a.strokeStyle=d;var h=canvasPointsMatrix[0][0],l=canvasPointsMatrix[0][e];h=getRecordXValue(h,60,f);l=getRecordYValue(l,b,c,g,15);a.beginPath();a.moveTo(h,l);for(d=1;d<canvasPointsMatrix.length;d++)h=canvasPointsMatrix[d][0],l=canvasPointsMatrix[d][e],h=getRecordXValue(h,
60,f),l=getRecordYValue(l,b,c,g,15),a.lineTo(h,l);a.stroke()}function getRecordXValue(a,b,c){return Math.round(a/b*c)}function getRecordYValue(a,b,c,d,e){return Math.round(d/2-a/b*(d/2-e)+(d-2*e)/12*c)}
