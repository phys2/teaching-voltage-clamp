...
function goldmanCurrentEquation(b,a,c,d,e){var f=Math.E,h=96.48533212*-b*c/2577.48341158;return(e-d*Math.pow(f,h))/(1-Math.pow(f,h))*Math.pow(b,2)*Math.pow(96.48533212,2)*c*a/2577.48341158}
function calculateCurrentForGivenPotential(b,a){var c=a[0],d=a[1],e=a[2],f=a[3],h=getSelectedIon("Na"),k=getSelectedIon("K"),l=getSelectedIon("Ca"),g=getSelectedIon("Cl");var m=$jscomp.makeIterator(getInternalConcentrations());var n=m.next().value;var p=m.next().value;var q=m.next().value;m=m.next().value;return c*(b-25.6925775528*Math.log(h/n))+d*(b-25.6925775528*Math.log(k/p))+e*(b-12.8462887764*Math.log(l/q))+f*(b-25.6925775528*Math.log(m/g))+getNoise(4)}
...
function getPermeabilities(b,a){switch($("#selectedChannel").val()){case "1":return[0,4,0,0];case "2":var c=4*(1-1/(1+Math.exp((b- -25)/-4))),d=0;null!=a&&(d=1/.86403262*(1-1/(1+Math.exp((a-d)/.05)))*Math.exp((-a+d)/10),c*=d);return[0,c,0,0];case "3":return c=4*(1-1/(1+Math.exp((b- -50)/4))),[0,c,0,0];case "8":return c=4*(1-1/(1+Math.exp((b- -50)/4))),d=.05,null!=a&&0!=a&&(d=1/.86403262*(1-1/(1+Math.exp((a-d)/.01)))*Math.exp((-a+d)/.4),c*=d),[0,c,0,0];case "4":return c=4*(1-1/(1+Math.exp((b- -45)/
6))),[c,0,0,0];case "9":return c=4*(1-1/(1+Math.exp((b- -45)/6))),d=.06,null!=a&&(d=1/.86403262*(1-1/(1+Math.exp((a-d)/.01)))*Math.exp((-a+d)/.04),c*=d),[c,0,0,0];case "7":return c=3*(1-1/(1+Math.exp((b- -15)/6))),d=.05,null!=a&&(d=1/.86403262*(1-1/(1+Math.exp((a-d)/.01)))*Math.exp((-a+d)/.6),c*=d),[0,0,c,0];case "6":return[0,0,0,1];case "5":return[2,0,0,0];case "10":return[1.5,1,0,0]}}
function getSelectedIon(b){b="#"+b+$("input[name=selectedSolution]:checked").val();return parseFloat($(b).val())}function updateBathReadout(){var b=timerCounterToTime(bathTimerCounter),a=getJunctionPotential(),c=parseFloat($("#offsetPotentialInput").val()),d=getNoise(.5);lastPotential=a=a+c+d;c=-1*a/.2;updateOutputFields(a,c);bathTimerCounter==6E4/deltaT?(bathTimerCounter=0,canvasPointsMatrix.length=0):(bathTimerCounter++,canvasPointsMatrix.push([b,a,c]),paintMainCanvas())}
